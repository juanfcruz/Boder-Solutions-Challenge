...
function createBorders(barrier){

    for (let i = 1; i < N - 1; i++) {

        barrier[IX(i, 0)]=true;
        barrier[IX(0, i)]=true;
        barrier[IX(N -1, i)]=true;
        barrier[IX(i, N -1)]=true;
...
function bounce_back(x, barrier)
{
    for (var i=1; i<N-1; i++){
        ...
        for(var j=1; j<N-1; j++){
            ...
            if(barrier[IX(i,j)]){
                ...
                x[IX(i,j+1)]= x[IX(i,j+1)]<0: -x[IX(i,j+1)]: x[IX(i,j+1)];

                x[IX(i,j-1)]= x[IX(i,j-1)]>0: -x[IX(i,j-1)]: x[IX(i,j-1)];

                x[IX(i+1,j)]= x[IX(i+1,j)]<0: -x[IX(i+1,j)]: x[IX(i+1,j)];
